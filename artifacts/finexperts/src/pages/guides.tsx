        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {guides.map((guide) => (
            <Link key={guide.slug} href={`/ghiduri/${guide.slug}`}>
              ...
              >
                <div className="h-48 overflow-hidden bg-[#E5E3D9] relative">
                  <img
                    src={guide.image}
                    alt={guide.title}
                    ...
                    onError={(e) => {
                      ...
                      if (parent) {
                        parent.style.background = "linear-gradient(135deg, #0A1A2E 0%, #1a3a5e 100%)";
                        const label = document.createElement("div");
                        label.className = "absolute inset-0 flex items-center justify-center";
                        label.innerHTML = `<span style="color:#C6A667;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;text-align:center;padding:16px">${guide.category}</span>`;
