          ...
          <div key={step.key} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${
                done ? "bg-green-500 text-white" : active ? "bg-[#0C1A2E] text-white ring-2 ring-[#C49A20] ring-offset-1" : "bg-[#E2E8F0] text-[#64748B]"
              }`}>
                {done ? <Check className="h-3 w-3" /> : i + 1}
              </div>
              <span className={`text-[9px] mt-1 font-medium whitespace-nowrap ${done || active ? "text-[#0C1A2E]" : "text-[#64748B]"}`}>{step.label}</span>
            </div>
...
export default function AccountPage() {
    ...
    return (
      <div className="min-h-screen bg-[#F5F7FA] flex items-center justify-center py-16 px-4">
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-10 text-center max-w-sm">
          <div className="w-14 h-14 rounded-full bg-[#0C1A2E]/8 flex items-center justify-center mx-auto mb-4">
            <FileText className="h-7 w-7 text-[#0C1A2E]" />
          </div>
          <h2 className="text-xl font-bold text-[#0C1A2E] mb-2">Contul meu</h2>
          <p className="text-sm text-[#64748B] mb-5">Conectează-te pentru a vedea aplicările tale și a gestiona ofertele.</p>
          <Link href="/login">
            <button className="bg-[#0C1A2E] text-white font-semibold px-6 py-2.5 rounded-lg text-sm w-full">Conectare</button>
          </Link>
          <p className="text-xs text-[#64748B] mt-3">Nu ai cont? <Link href="/register" className="text-[#0C1A2E] font-semibold">Înregistrare</Link></p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA]">
      {/* Header */}
      <div className="bg-[#0C1A2E] py-7">
        <div className="max-w-5xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-[#C49A20] flex items-center justify-center text-[#0C1A2E] font-bold text-base">{initials}</div>
            <div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 bg-white border border-[#E2E8F0] rounded-xl p-1 mb-6 overflow-x-auto">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-1.5 flex-shrink-0 px-3 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === t.id ? "bg-[#0C1A2E] text-white" : "text-[#64748B] hover:text-[#0C1A2E]"}`}>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <button key={s.label} onClick={() => setActiveTab(s.tab)} className="bg-white border border-[#E2E8F0] rounded-xl p-5 text-center hover:border-[#0C1A2E] transition-colors">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-xl p-5">
              <h3 className="font-semibold text-[#0C1A2E] mb-4">Acțiuni rapide</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  { label: "Aplică credit", desc: "Personal sau ipotecar", icon: FileText, href: "/aplica", bg: "bg-[#0C1A2E]/8", ic: "text-[#0C1A2E]" },
                  ...
                  a.href ? (
                    <a key={a.label} href={a.href} className="flex items-center gap-3 p-4 border border-[#E2E8F0] rounded-xl hover:border-[#0C1A2E] transition-colors group">
                      <div className={`w-9 h-9 rounded-lg ${a.bg} flex items-center justify-center shrink-0`}><a.icon className={`h-4 w-4 ${a.ic}`} /></div>
                      <div><div className="text-sm font-semibold text-[#0C1A2E]">{a.label}</div><div className="text-xs text-[#64748B]">{a.desc}</div></div>
                    </a>
                  ) : (
                    <button key={a.label} onClick={() => a.tab && setActiveTab(a.tab)} className="flex items-center gap-3 p-4 border border-[#E2E8F0] rounded-xl hover:border-[#0C1A2E] transition-colors text-left">
                      <div className={`w-9 h-9 rounded-lg ${a.bg} flex items-center justify-center shrink-0`}><a.icon className={`h-4 w-4 ${a.ic}`} /></div>
                      <div><div className="text-sm font-semibold text-[#0C1A2E]">{a.label}</div><div className="text-xs text-[#64748B]">{a.desc}</div></div>
                    </button>
                  )
            ...
            {myApplications.length > 0 && (
              <div className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-[#E2E8F0] flex items-center justify-between">
                  <h3 className="font-semibold text-[#0C1A2E]">Aplicările mele</h3>
                  <button onClick={() => setActiveTab("credit")} className="text-xs text-[#64748B] hover:text-[#0C1A2E]">Toate →</button>
                </div>
                {myApplications.slice(0, 2).map(app => (
                  <div key={app.id} className="px-5 py-4 border-b border-[#E2E8F0] last:border-b-0">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <div className="text-sm font-semibold text-[#0C1A2E]">{app.type === "personal" ? "Credit personal" : app.type === "ipotecar" ? "Credit ipotecar" : "Refinanțare"}</div>
            ...
            {/* Broker CTA */}
            <div className="bg-[#0C1A2E] rounded-xl p-6 flex items-center justify-between gap-4">
              <div>
              ...
              <div className="flex flex-col gap-2 shrink-0">
                <a href="tel:0799715101">
                  <button className="bg-[#C49A20] hover:bg-[#b09255] text-[#0C1A2E] font-semibold text-sm py-2.5 px-5 rounded-xl transition-colors flex items-center gap-2 whitespace-nowrap">
        ...
        {activeTab === "credit" && (
          <div>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-bold text-[#0C1A2E]">Aplicările mele</h2>
              <Link href="/aplica">
                <button className="flex items-center gap-1.5 bg-[#0C1A2E] text-white font-semibold px-4 py-2 rounded-lg text-sm">
            ...
            {myApplications.length === 0 ? (
              <div className="bg-white border border-[#E2E8F0] rounded-xl p-12 text-center">
                ...
                <h3 className="font-semibold text-[#0C1A2E] mb-2">Nicio aplicare încă</h3>
                <p className="text-sm text-[#64748B] mb-5">Aplică pentru primul tău credit prin FinExperts.</p>
                <Link href="/aplica">
                  <button className="bg-[#0C1A2E] text-white font-semibold px-5 py-2.5 rounded-lg text-sm inline-flex items-center gap-2">
              ...
              <div className="space-y-4">
                {myApplications.map(app => (
                  <div key={app.id} className="bg-white border border-[#E2E8F0] rounded-xl p-5">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <div className="font-semibold text-[#0C1A2E] mb-0.5">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      <div><div className="text-xs text-[#64748B] mb-0.5">Sumă</div><div className="text-sm font-semibold text-[#0C1A2E]">{app.amount.toLocaleString("ro-RO")} RON</div></div>
                      <div><div className="text-xs text-[#64748B] mb-0.5">Bancă</div><div className="text-sm text-[#0C1A2E]">{app.bank || "—"}</div></div>
                ...
                <Link href="/aplica">
                  <button className="w-full border-2 border-dashed border-[#E2E8F0] rounded-xl py-4 text-sm text-[#64748B] hover:border-[#0C1A2E] hover:text-[#0C1A2E] transition-colors flex items-center justify-center gap-2">
        ...
        {activeTab === "asigurari" && (
          <div>
            <div className="mb-5">
              <h2 className="text-xl font-bold text-[#0C1A2E]">Asigurările mele</h2>
                ...
                return (
                  <div key={ins.id} className="bg-white border border-[#E2E8F0] rounded-xl p-5">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-10 h-10 rounded-lg bg-[#0C1A2E]/8 flex items-center justify-center shrink-0">
                        <Icon className="h-5 w-5 text-[#0C1A2E]" />
                      </div>
                      <div className="flex-1">
                        <div className="font-semibold text-[#0C1A2E] mb-0.5">{ins.title}</div>
                        <div className="text-xs text-[#64748B]">{ins.desc}</div>
                      </div>
                      <div className="text-right shrink-0">
                        <div className="text-xs text-[#64748B]">De la</div>
                        <div className="font-bold text-[#0C1A2E]">{ins.price} RON/an</div>
                      </div>
                    </div>
                    {applyingIns === ins.id ? (
                      <div className="bg-[#F5F7FA] rounded-xl p-4 space-y-3">
                        <div className="text-xs font-semibold text-[#0C1A2E] uppercase tracking-wider">Solicită ofertă</div>
                        <input type="text" placeholder="Numele tău" value={insForm.name} onChange={e => setInsForm(f => ({ ...f, name: e.target.value }))}
                          className="w-full border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:border-[#0C1A2E]" />
                        <input type="tel" placeholder="Telefon" value={insForm.phone} onChange={e => setInsForm(f => ({ ...f, phone: e.target.value }))}
                          className="w-full border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:border-[#0C1A2E]" />
                        <div className="flex gap-2">
                          <button onClick={() => submitInsurance(ins.title)} className="flex-1 bg-[#0C1A2E] text-white font-semibold py-2 rounded-lg text-sm">Trimite</button>
                      ...
                      <button onClick={() => handleInsuranceApply(ins.id)} className="w-full border border-[#E2E8F0] hover:border-[#0C1A2E] text-[#0C1A2E] font-medium py-2 rounded-lg text-sm flex items-center justify-center gap-1.5 transition-colors">
            ...
            <div className="mt-5 bg-[#0C1A2E] rounded-xl p-5 flex items-center justify-between gap-4">
              <div>
        ...
        {activeTab === "documente" && (
          <div>
            <div className="mb-5">
              <h2 className="text-xl font-bold text-[#0C1A2E]">Documente necesare</h2>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden mb-4">
              <div className="px-5 py-4 border-b border-[#E2E8F0] flex items-center justify-between">
                <h3 className="font-semibold text-[#0C1A2E]">Checklist dosar credit</h3>
                <span className="text-sm font-bold text-[#0C1A2E]">{checkedDocs.size}/{documentChecklist.length} pregătite</span>
              </div>
              <div className="p-2">
                {/* Progress bar */}
                ...
                {documentChecklist.map((doc, i) => (
                  <button key={i} onClick={() => toggleDoc(i)}
                    ...
                    <div className="flex-1">
                      <div className={`text-sm font-medium ${checkedDocs.has(i) ? "line-through text-[#64748B]" : "text-[#0C1A2E]"}`}>{doc.label}</div>
            ...
            <div className="bg-[#0C1A2E] rounded-xl p-5 flex items-center justify-between gap-4">
              <div>
        ...
        {activeTab === "profil" && (
          <div>
            <div className="mb-5">
              <h2 className="text-xl font-bold text-[#0C1A2E]">Profilul meu</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_300px] gap-5">
              <div className="bg-white border border-[#E2E8F0] rounded-xl p-6">
                <h3 className="font-semibold text-[#0C1A2E] mb-5">Informații personale</h3>
                {profileSaved && (
                ...
                <div className="space-y-4">
                  ...
                  <button onClick={() => { setProfileSaved(true); setTimeout(() => setProfileSaved(false), 3000); }}
                    className="bg-[#0C1A2E] text-white font-semibold px-5 py-2.5 rounded-lg text-sm flex items-center gap-2">
              ...
              <div className="space-y-4">
                <div className="bg-white border border-[#E2E8F0] rounded-xl p-5">
                  <div className="w-16 h-16 rounded-full bg-[#C49A20] flex items-center justify-center text-[#0C1A2E] font-bold text-xl mx-auto mb-3">{initials}</div>
                  <div className="text-center">
                    <div className="font-semibold text-[#0C1A2E]">{user?.name}</div>
                ...
                <div className="bg-[#F5F7FA] border border-[#E2E8F0] rounded-xl p-4">
                  <div className="text-xs font-semibold text-[#0C1A2E] uppercase tracking-wider mb-3">Brokerul tău</div>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-full bg-[#0C1A2E] flex items-center justify-center text-white font-bold text-sm">AA</div>
                    <div>
                      <div className="text-sm font-semibold text-[#0C1A2E]">Alexandra Achim</div>
                      <div className="text-xs text-[#64748B]">Broker senior</div>
                    </div>
                  </div>
                  <a href="tel:0799715101" className="flex items-center gap-2 text-xs text-[#0C1A2E] hover:text-[#C49A20] transition-colors mb-1">
                    <Phone className="h-3.5 w-3.5" /> 0799 715 101
                  </a>
                  <a href="mailto:alexandra.achim@kiwifinance.ro" className="flex items-center gap-2 text-xs text-[#0C1A2E] hover:text-[#C49A20] transition-colors">
